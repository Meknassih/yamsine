...
export function DiceArea({ game, clientId, onRoll }: Props) {
  ...
  // Local kept state — reset whenever server sends new dice values
  const [localKept, setLocalKept] = useState<boolean[]>(
    game.dice.map((d) => d.kept)
  );

  useEffect(() => {
    setLocalKept(game.dice.map((d) => d.kept));
  }, [game.dice]);

  function toggleKeep(index: number) {
    if (!isMyTurn || game.rollsLeft >= 3 || game.rollsLeft === 0) return;
    setLocalKept((prev) => prev.map((k, i) => (i === index ? !k : k)));
  }
